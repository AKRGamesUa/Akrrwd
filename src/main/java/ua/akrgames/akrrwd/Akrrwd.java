...
public class Akrrwd extends JavaPlugin {
    ...
    // Valid languages - using Arrays.asList for compatibility with Java 8+ / MC 1.16+
    private static final List<String> VALID_LANGUAGES = Arrays.asList("en", "ua", "pl");
    ...
    public void onEnable() {
        ...
        particleManager.loadTemplatesFile();
        loadConfig();
        setupCooldownFile();
        ...
        if (bStatsEnabled) {
            Metrics metrics = new Metrics(this, 26837);
            getLogger().info("bStats metrics enabled.");
        } else {
            getLogger().info("bStats metrics disabled in config.");
        }

        // Start async update checker
    ...
    private void loadConfig() {
        ...
        cooldownTime = config.getLong("cooldown", 3600) * 1000;
        ...
        particleManager.loadConfig(config);
        particleManager.reloadTemplatesFile();
        ...
        cooldownExpiredMessage        = messagesConfig.getString(language + ".cooldown-expired",
                "<GRADIENT:#00FF00>%player%, your cooldown has expired! Use /randomreward to claim a reward!</GRADIENT:#00FFFF>");
        ...
        String failureSoundName = config.getString("sounds.failure.sound", "BLOCK_NOTE_BLOCK_BASS");
        try {
            failureSound = Sound.valueOf(failureSoundName.toUpperCase());
        } catch (IllegalArgumentException e) {
            getLogger().warning("Invalid failure sound: " + failureSoundName + ". Defaulting to BLOCK_NOTE_BLOCK_BASS.");
            failureSound = Sound.BLOCK_NOTE_BLOCK_BASS;
        }
    ...
    /**
     * Public accessor so UpdateChecker (and other classes) can use the
     * language-aware message system without duplicating logic.
     */
    public String getMessagePublic(String key, String... replacements) {
    ...
    private String processGradients(String message) {
        ...
        while (matcher.find()) {
            ...
            result.append(createGradient(text, startHex, endHex));
            lastEnd = matcher.end();
        }
        result.append(message.substring(lastEnd));
        return result.toString();
    ...
    class RandomRewardCommand implements CommandExecutor {
        ...
        public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
            ...
            particleManager.spawnRewardParticles(player);
    ...
    class ReloadCommand implements CommandExecutor {
        ...
        public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
            ...
            if (args[0].equalsIgnoreCase("reload")) {
                ...
                setupMessagesFile();
                setupCooldownFile();
                loadConfig(); // also reloads particleManager config + templates
            ...
            if (args[0].equalsIgnoreCase("reset") && args.length == 2) {
                ...
                Player onlinePlayer = getServer().getPlayerExact(target);
                String uuid = null;
                if (onlinePlayer != null) {
                    uuid = onlinePlayer.getUniqueId().toString();
                } else {
                    for (String key : cooldownConfig.getKeys(false)) {
                        if (cooldownConfig.getString(key + ".playerName", "").equalsIgnoreCase(target)) {
                            uuid = key;
                            break;
                        }
                    }
                ...
                if (uuid == null) {
                    sender.sendMessage(getMessage("player-not-found", "%player%", target));
                    return true;
                }
                cooldownConfig.set(uuid, null);
                saveCooldownFile();
                bossBarManager.removeBossBar(onlinePlayer);
                sender.sendMessage(getMessage("cooldown-reset", "%player%", target));
                return true;
            }
